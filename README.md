...
- **Multi-tenancy via user_id + RLS.** Every clinical row is scoped to the authenticated doctor. Standard Supabase pattern: `user_id UUID REFERENCES auth.users(id)` + RLS policy `WHERE user_id = auth.uid()`.
